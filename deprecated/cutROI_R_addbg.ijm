...
//Macro to cut out 16 bit stacks from ROI's 
...
for (k=0;k<list.length;k++){
	...
	//iterate thru each ROI in the roi file			
	n = roiManager("count");
		for (m=0;m<n;m++){
			roiManager("Select", m);
			newImage("CELL_CROPPED","16-bit Black",250,250,nSlices);
			//Set background of new image to min value of whole stack
			for (s = 1; s <= nSlices; s++) {
			setSlice(s);
			changeValues(0,0,min);
			}			
					
			final = getImageID;			

			//Copy and paste ROI from ORIGINALto new image
			for (s = 1; s <= nSlices; s++) {
